...
echo ""
echo "Or install the systemd service (substitute user and project path):"
echo "  sed -e \"s|@USER@|\$(id -un)|g\" \\"
echo "      -e \"s|@GROUP@|\$(id -gn)|g\" \\"
echo "      -e \"s|@PROJECT_DIR@|$PROJECT_DIR|g\" \\"
echo "      deploy/laser-detection.service | sudo tee /etc/systemd/system/laser-detection.service"
echo "  sudo systemctl daemon-reload"
echo "  sudo systemctl enable --now laser-detection"
echo ""
echo "Verify NPU and FastSAM after service start:"
echo "  curl -s http://localhost:8000/health | jq '.npu.blockers, .npu.models, .detection.fastsam'"
echo "  # Expect npu.blockers: [], fastsam loaded with device: hailo"
